...
get(Key, Time) when is_integer(Time) ->
    ...
    case ets:lookup(?MODULE, {Key, Time}) of
...
init(_) ->
    ...
    timer:apply_interval(1000, ?MODULE, collect_sample, []).
...
to_json_term(Agg) ->
    ...
    {[
        ...
        {sum, Agg#aggregate.sum},
        {mean, Agg#aggregate.mean},
        {stddev, Agg#aggregate.stddev},
        {min, Min},
        {max, Max}
    ]}.

clamp_value(Val) when Val > 0.00000000000001 -> Val;
clamp_value(_) -> 0.0.
